...
public class BioClient {
    public static void main(String[] args) {
        ...
        for (int index = 0; index < clientNumber; index++, countDownLatch.countDown()) {
            new Thread(new BioClientThread(countDownLatch, index)).start();
        }
    }
}

class BioClientThread implements Runnable {
    private final CountDownLatch countDownLatch;

    private final Integer clientIndex; //这个线程的编号

    //countDownLatch是java提供的同步计数器。当计数器数值减为0时，所有受其影响而等待的线程将会被激活。这样保证模拟并发请求的真实性
    public BioClientThread(CountDownLatch countDownLatch , Integer clientIndex) {
    ...
    public void run() {
        ...
        try {
            ...
            System.out.println("From Server：第" + this.clientIndex + "个客户端接收到来自服务器的信息:" + message);
        } catch (Exception e) {
